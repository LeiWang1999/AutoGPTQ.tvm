                ...
                for (k_0_1: int32, 0, 4) {
                  ...
                  for (ax0_0_1: int32, 0, 8) {
                    for (ax1_0_1: int32, 0, 1) {
                      block([1024, 1024], "B_decompress_shared_wmma.matrix_b_o") as [v0_o_1, v1_o_1] {
                        bind(v0_o_1, (((j_0_0_0*64) + (j_0_0_1*8)) + ax0_0_1))
                        bind(v1_o_1, (((k_0_0*4) + k_0_1) + ax1_0_1))
                        tir.reads([B_decompress_shared[(v0_o_1*16):((v0_o_1*16) + 16), (v1_o_1*16):((v1_o_1*16) + 16)]])
                        tir.writes([B_decompress_shared_wmma.matrix_b[(v0_o_1*16):((v0_o_1*16) + 16), (v1_o_1*16):((v1_o_1*16) + 16)]])
                        A_4 = match_buffer(B_decompress_shared[(v0_o_1*16):((v0_o_1*16) + 16), (v1_o_1*16):((v1_o_1*16) + 16)])
                        C_6 = match_buffer(B_decompress_shared_wmma.matrix_b[(v0_o_1*16):((v0_o_1*16) + 16), (v1_o_1*16):((v1_o_1*16) + 16)])
                  ...
                  for (i_0_2: int32, 0, 1) {
                    for (j_0_2: int32, 0, 8) {
                      block([1024, 1024, tir.reduce_axis(0, 1024)], "B_update_o") as [vi_o_1, vj_o_1, vk_o] {
                        bind(vi_o_1, ((i_0_0 + i_0_1) + i_0_2))
                        bind(vj_o_1, ((((j_0_0_0*64) + (j_0_0_1*8)) + (j_0_1*8)) + j_0_2))
                        bind(vk_o, ((k_0_0*4) + k_0_1))
                        tir.reads([C_wmma.accumulator[(vi_o_1*16):((vi_o_1*16) + 16), (vj_o_1*16):((vj_o_1*16) + 16)], A_shared_wmma.matrix_a[(vi_o_1*16):((vi_o_1*16) + 16), (vk_o*16):((vk_o*16) + 16)], B_decompress_shared_wmma.matrix_b[(vj_o_1*16):((vj_o_1*16) + 16), (vk_o*16):((vk_o*16) + 16)]])
                        ...
                        B_2 = match_buffer(B_decompress_shared_wmma.matrix_b[(vj_o_1*16):((vj_o_1*16) + 16), (vk_o*16):((vk_o*16) + 16)])
              ...
              for (ax0_0_2: int32, 0, 1) {
                for (ax1_0_2: int32, 0, 8) {
                  block([1024, 1024], "C_wmma.accumulator_o") as [v0_o_2, v1_o_2] {
                    bind(v0_o_2, (i_0_0 + ax0_0_2))
                    bind(v1_o_2, (((j_0_0_0*64) + (j_0_0_1*8)) + ax1_0_2))
